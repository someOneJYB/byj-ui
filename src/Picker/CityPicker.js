...
class CityPicker extends React.Component {
    ...
    getProvince = (province) => {
        const v = this.props.data.filter((i) => i.value === province)[0];
...
return v ? v : this.props.data[0];
    ...
    getCity = (provinces, city) => {
        const v = provinces.children.filter((i) => i.value === city)[0];
...
return v ? v : provinces.children[0];
    ...
    getDistrict = (city, district) => {
        const v = city.children.filter((i) => i.value === district)[0];
...
return v ? v : city.children[0];
    ...
    initValue = () => {
        ...
        const v1 = this.getProvince(value[0]);
        const v2 = this.getCity(v1, value[1]);
        const v3 = this.getDistrict(v2, value[2]);
        this.setState({
    ...
    change = (ind, index, value) => {
        ...
        if (ind === 0) {
            ...
            //     }
            // });
            const d1 = data.filter((i) => i.value === value.value);
            // 城市发生变化
        ...
        if (ind === 1) {
            const d1 = data.filter((i) => i.value === this.state.value[0]);
            const d2 = d1[0].children.filter((i) => i.value === value.value);
            // console.log('d2[0]', d2)
        ...
        if (ind === 2) {
            const v1 = this.getProvince(this.state.value[0]);
            const v2 = this.getCity(v1, this.state.value[1]);
            const v3 = this.getDistrict(v2, value.value);
