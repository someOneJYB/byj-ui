...
[
{ 'label': '夜的第一张',
'value': '夜的第一张' }, { 'label': '夜的第二张',
'value': '夜的第二张' }, { 'label': '夜的第三张',
'value': '夜的第三张' }, { 'label': '夜的第四张',
'value': '夜的第四张' }, { 'label': '夜的第五张',
'value': '夜的第五张' }, { 'label': '夜的第六张',
'value': '夜的第六张' }, { 'label': '夜的第八张',
'value': '夜的第八张' }
], [
{ 'label': '春',
'value': '春' }, { 'label': '夏',
...
'value': '冬' }
    ...
    static getDerivedStateFromProps(props, state) {
        if (props.data !== state.data && props.data) {
            const v = initActiveIndex(props.value, props.data)
...
return <div>
            ...
            <div className="picker-all">
                ...
                {
                    data.map((item, ind) => <div key={ind} className="picker-con" style={{ 'height': `${data[ind].length * 30}px`,
'transform': `translate3d(0, ${(2 - this.state[`activeIndex${ind}`]) * 30}px, 0)` }} onTouchStart={(e) => this.start(e, ind)} onTouchEnd={(e) => this.end(e, ind)}>
                                {
                                    item.map((it) => <div key={it.label} className="picker-item">{it.label}</div>)
                                }
                            </div>)
                }
            </div>
...
MultyPicker.propTypes = {
    'visible': PropTypes.bool,
    'data': PropTypes.array,
    'onChoose': PropTypes.func,
    'value': PropTypes.array
}
MultyPicker.defaultProps = {
    'visible': true,
    'data': []
}
