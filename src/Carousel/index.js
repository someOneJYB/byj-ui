...
* 需要明确的
* */
class SilederLeft extends Component {
   constructor(props) {
       super(props)
       this.timer =  null;
       this.state ={
    ...
    render() {
       ...
       return (
           <div className={`con  ${parentClass}`} style={style} onTouchStart={this.touchStart} onTouchEnd={this.touchEnd} onTouchCancel={this.touchCancel}>
               <div className="bag" style={{ width: parseInt(width+2)*length+'px', transition: `transform ${duration}s ease-out`, transform: `translateX(${direction})`, ...extraStyle}}>
                   <div key={imageList.length} className={`son  ${item.classy}`} style={{ backgroundImage:`url(${item.url})`, ...item.style, width,}}>{item.children||null}</div>
                {
                    imageList.map(({ url = '', style = {}, classy = '', children }, index) => {
                        return <div key={url ||  index} className={`son  ${classy}`} style={{ backgroundImage:`url(${url})`, ...style, width,}}>{children||null}</div>
                    })
                }
                   <div key={-1} className={`son  ${item1.classy}`} style={{ backgroundImage:`url(${item1.url})`, ...item1.style, width,}}>{item1.children||null}</div>
               </div>
               {
                   dot &&  <ul className="dot-con">
                       {
                           imageList.map((item, index) => {
                                return <li onClick={e => this.goto(index)} key={index} className={`dot-icon ${activeIndex%length === index ? 'active' : ''}`}></li>
                           })
                       }
                   </ul>
               }
